...
namespace TorannMagic
{
    public static class TM_Calc
    {
        ...
        public static bool IsWanderer(Pawn pawn)
        {
            ...
            // Cannot enter infinite loop in IsMagic/MightUser because a pawn must be mighty from something other than Field Training
            if (
                pawn.story.traits.HasTrait(TorannMagicDefOf.TM_Wayfarer)
                || (
                    comp.customClass?.classFighterAbilities != null
                    && comp.customClass.classFighterAbilities.Contains(TorannMagicDefOf.TM_FieldTraining)
                )
            ) //pawn is a wayfarer with appropriate skill level
            {
        ...
        public static bool IsWayfarer(Pawn pawn)
        {
            ...
            // Cannot enter infinite loop in IsMagic/MightUser because a pawn must be magic from something other than Cantrips
            if (
                pawn.story.traits.HasTrait(TorannMagicDefOf.TM_Wanderer)
                || (
                    comp.customClass?.classMageAbilities != null
                    && comp.customClass.classMageAbilities.Contains(TorannMagicDefOf.TM_Cantrips)
                )
            ) //pawn is a wanderer with appropriate skill level
            {
        ...
        //Rand.Chance(((ModOptions.Settings.Instance.baseFighterChance * 6) + (ModOptions.Settings.Instance.baseMageChance * 6) + (8 * ModOptions.Settings.Instance.advFighterChance) + (16 * ModOptions.Settings.Instance.advMageChance)) / (allTraits.Count))
