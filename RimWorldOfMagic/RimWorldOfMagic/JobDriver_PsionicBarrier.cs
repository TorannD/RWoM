                ...
                for (int j = 0; j < cellList.Count; j++)
                {
                    if (cellList[j] is Projectile)
                    {
                        ...
                        if (p.Launcher != null && p.Launcher.Position.DistanceTo(TargetLocA) > (radius +.5f))
                        {
                            ...
                            {
                                GenExplosion.DoExplosion(barrierCells[i], this.pawn.Map, cellList[j].def.projectile.explosionRadius, cellList[j].def.projectile.damageDef, this.pawn, (int)projectileDamage, cellList[j].def.projectile.GetArmorPenetration(null, null), cellList[j].def.projectile.soundExplode,
                                    null, cellList[j].def, null, cellList[j].def.projectile.postExplosionSpawnThingDef, cellList[j].def.projectile.postExplosionSpawnChance, cellList[j].def.projectile.postExplosionSpawnThingCount, null, null, 0, cellList[j].def.projectile.applyDamageToExplosionCellsNeighbors,
