    ...
    public class Verb_SummonTotemLightning : Verb_UseAbility
    {
        ...
        protected override bool TryCastShot()
        {
            ...
            if (shiftPos != null && (shiftPos.IsValid && shiftPos.Standable(map)))
            {
                ...
                try
                {
                    this.totem = TM_Action.SingleSpawnLoop(caster, tempPod, shiftPos, map, 2500 + (125 * verVal), true, false, caster.Faction, false);
                    this.totem.SetFaction(caster.Faction);
                    Building_TMTotem_Lightning totemBuilding = this.totem as Building_TMTotem_Lightning;
                ...
                {
                    comp.Mana.CurLevel += comp.ActualManaCost(TorannMagicDefOf.TM_SummonTotemLightning);
            ...
            else
            {
                if (caster.IsColonist)
                {
                    ...
                    comp.Mana.GainNeed(comp.ActualManaCost(TorannMagicDefOf.TM_SummonTotemLightning));
                }
            }
            return false;
        }
