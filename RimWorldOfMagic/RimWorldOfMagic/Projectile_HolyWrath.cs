        ...
        protected override void Impact(Thing hitThing, bool blockedByShield = false)
        {
            ...
            for (int j = 0; j < smitePos.Count; j++)
            {
                if (wrathAge[j] == this.timeToSmite/strikeNum)
                {
                    ...
                    CompAbilityUserMagic comp = caster.GetCompAbilityUserMagic();
                    ExplosionHelper.Explode(smitePos[j], map, 3f, TMDamageDefOf.DamageDefOf.TM_Overwhelm, this.launcher as Pawn, Mathf.RoundToInt((12 + TMDamageDefOf.DamageDefOf.TM_Overwhelm.defaultDamage + 3*pwrVal) * this.arcaneDmg), 0, TorannMagicDefOf.TM_Lightning, def, this.equipmentDef, null, null, 0f, 1, null, false, null, 0f, 1, 0f, false);
                }
            }
        }
