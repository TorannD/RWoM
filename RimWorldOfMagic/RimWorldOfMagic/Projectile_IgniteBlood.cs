    ...
    public class Projectile_IgniteBlood : Projectile_AbilityBase
	{
        ...
        public void BurnBloodAtCell()
        {
            for (int i = 0; i < this.BF.Count; i++)
            {
                ...
                ExplosionHelper.Explode(this.BF[i].position, this.Map, .2f + (.4f * BF[i].pulseCount), TMDamageDefOf.DamageDefOf.TM_BloodBurn, this.launcher, Mathf.RoundToInt((Rand.Range(2.8f, 4.5f) * (1 + (.12f * pwrVal))) * this.arcaneDmg), .5f, TorannMagicDefOf.TM_FireWooshSD, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0.0f, false);
