    ...
    public class Verb_AnimalFriend : Verb_UseAbility
    {
        ...
        protected override bool TryCastShot()
        {
            ...
            if(animal !=null && animal.RaceProps.Animal && animal.RaceProps.IsFlesh)
            {
                ...
                float animalWildness = animal.def.GetStatValueAbstract(StatDefOf.Wildness);
                if (animal == comp.bondedPet)
                {
                    Hediff animalBondHD = oldbond.health.hediffSet.GetFirstHediffOfDef(HediffDef.Named("TM_RangerBondHD"));
                    if (animalBondHD != null && Rand.Chance(((1f + animalBondHD.ageTicks / 6000000) - animalWildness)))
                    {
                ...
                else
                {
                    ...
                    {
                        if ((animalWildness <= .7f) || (animalWildness <= .8f && pwr.level == 1) || (animalWildness <= .9f && pwr.level == 2) || pwr.level == 3)
                        {
                            if (Rand.Chance(.6f + (.05f *pwr.level)) && Rand.Chance(((.7f + (.1f *pwr.level)) - animalWildness) * 10))
                            {
                            ...
                            else
                            {
                                ...
                                animal.LabelShort,
                                pawn.LabelShort,
                                (Mathf.Clamp(Mathf.RoundToInt(((.7f + (.1f * pwr.level)) - animalWildness) * 1000f), 0, 100 - (40 - (5 * pwr.level))))
                                ), MessageTypeDefOf.NeutralEvent);
