    ...
    public class FlyingObject_LightningTrap : Projectile
    {
        ...
        public void SearchForTargets(IntVec3 center, float radius)
        {
            ...
            if (target != null)
            {
                ...
                for (int k = 0; k < Rand.Range(1, 5); k++)
                {
                    ...
                    GenExplosion.DoExplosion(randomCell, base.Map, Rand.Range(.4f, .8f), TMDamageDefOf.DamageDefOf.TM_Lightning, this.launcher, Mathf.RoundToInt(Rand.Range(4, 6)), 0, SoundDefOf.Thunder_OnMap, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0.1f, true);
                }
                GenExplosion.DoExplosion(target.Position, base.Map, 1f, TMDamageDefOf.DamageDefOf.TM_Lightning, this.launcher, Mathf.RoundToInt(Rand.Range(5, 9) * this.arcaneDmg), 0, SoundDefOf.Thunder_OffMap, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0.1f, true);
        ...
        protected new void Impact(Thing hitThing)
        {
            ...
            if (hasValue)
            {
                ...
                if (flag4)
                {
                    GenExplosion.DoExplosion(this.origin.ToIntVec3(), base.Map, 0.9f, DamageDefOf.Stun, this, -1, 0, null, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0f, false);
                }
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (strikeCell.InBoundsWithNullCheck(base.Map) && strikeCell.IsValid && !strikeCell.Fogged(this.Map))
                {
                    ...
                    for (int k = 0; k < Rand.Range(1, 8); k++)
                    {
                        ...
                        cellRect.ClipInsideMap(base.Map);
                        IntVec3 randomCell = cellRect.RandomCell;
                        GenExplosion.DoExplosion(randomCell, base.Map, Rand.Range(.2f, .6f), TMDamageDefOf.DamageDefOf.TM_Lightning, this.launcher, Mathf.RoundToInt(Rand.Range(2, 6)), 0, SoundDefOf.Thunder_OffMap, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0.1f, true);
                    }
                }
            }
            GenExplosion.DoExplosion(this.origin.ToIntVec3(), base.Map, 1f, TMDamageDefOf.DamageDefOf.TM_Lightning, this.launcher, Mathf.RoundToInt(Rand.Range(4, 8)), 0, SoundDefOf.Thunder_OffMap, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0.1f, true);
